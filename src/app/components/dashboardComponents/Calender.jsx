...
const Calender = ({ user, solvedProblems, targetUserDashboard }) => {
  ...
  const validUser = targetUserDashboard;
  ...
  const tileClassName = ({ date, view }) => {
    if (view === "month") {
      ...
      if (count) {
        if (count >= 3)
          return "bg-green-700 text-white rounded-md"; // More intense green for 3+
        else if (count >= 2)
          return "bg-green-600 text-white rounded-md"; // Medium green for 2
  ...
  const currentUserSolvedCount = user?.solvedCount ||
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-center items-center lg:items-stretch w-full p-4 lg:p-8  space-y-2 lg:space-x-8   ">
      ...
      >
        ...
        <div className="w-full border-t border-gray-800 dark:border-gray-200 pt-2 mt-8">
          {" "}
          {/* Separator */}
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4 text-center">
            Points System
          ...
          <div className="grid grid-cols-3 gap-4 text-center mb-6">
            ...
            <div className="flex flex-col items-center p-3 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700">
              <span className="text-lg font-bold text-yellow-700 dark:text-yellow-300">
                {currentUserSolvedCount.medium || 0}
              </span>
              <span className="text-xs text-gray-600 dark:text-gray-400">
                Medium Solved
              </span>

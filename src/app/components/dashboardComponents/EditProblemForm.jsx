...
const EditProblemForm = ({ closeModal, problem, triggerChange }) => {
  ...
  const {
    ...
    defaultValues: {
      problemStatement: problem.problemName,
      problemLink: problem.problemLink,
      difficulty: problem.difficulty,
      programmingLanguage: problem.solutions[0].languageUsed,
  ...
  const mySubmit = async (data) => {
    ...
    try {
      const response = await problemServices.edit(
        problem.id,
        {
  ...
  return (
    <div className="w-full flex justify-center items-center dark:text-white fixed inset-0 backdrop-blur-xl z-50 bg-black/30">
      <form
        ...
        <div className="flex justify-center items-center gap-2 w-1/w">
          ...
          <div className="w-1/2 flex items-center justify-center">
            <button
              ...
              className="cursor-pointer bg-blue-300 hover:bg-blue-500 dark:bg-blue-700 dark:hover:bg-blue-900 m-1 py-1 md:px-4 px-2 rounded-lg shadow-lg"
            >
              {currentStep === totalSteps - 2 ? "Update" : "Next"}
            </button>
          </div>
          {apiError && (
            <div>
